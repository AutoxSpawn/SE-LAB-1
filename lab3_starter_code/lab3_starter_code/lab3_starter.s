...
``` #you can change this, but in my implmentation I saved s0 and ra on the stack
    ...
    dec_counter:
        ...
        li t0, MIN_VAL # Loading the minimum allowed value (0) into t0
        beq s0, t0, err_counter # if current counter is equal to 0, you go to error code (flash LEDs)
        addi s0, s0, -1 # Otherwise, you will decrement the counter by 1, s0 -= 1
        mv a0, s0 # move the updated counter value into a0
    ...
    inc_counter:
        ...
        li t0, MAX_VAL # Load the max allowed value (99) into t0
        beq s0, t0, err_counter # If the counter is 99, jump to err_counter
        addi s0, s0, 1 # Otherwise, increment the counter by 1, s0 += 1
        mv a0, s0 # move the updated counter value into a0
...
write_lcd:
    ...
    jal extract_bcd # Split the number into tens(a0) and ones(a1)
    jal encode_digit # convert the two digits into 7-segment bit pattern, which is stored in t0

    li a0, 0x120 # Load the ecall code for updating the 7-segment display
    mv a1, t0 # move the final what needs to be display into a1
    lw a2, digit_msk # load the bitmask, which is the parts of the display that needs to be updated
    ecall # call the simulator to update the display
